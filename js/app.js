...
document.addEventListener("DOMContentLoaded", () => {
  Promise.all([
    ...
    fetch("component/home/program_dan_materi/program_dan_materi.html").then(
      (res) => res.text(),
    ),
    fetch("component/home/statistik_dan_status_pendaftaran.html").then((res) =>
      res.text(),
    ),
    fetch("component/home/daftar_program_tersedia.html").then((res) =>
      res.text(),
    ),
    fetch("component/home/faq.html").then((res) => res.text()),
    fetch("component/home/program_dan_materi/task/sesi_1_persiapan.html").then(
      (res) => res.text(),
    ),
    fetch("component/home/program_dan_materi/task/sesi_2_soal.html").then(
      (res) => res.text(),
    ),
    fetch("component/home/program_dan_materi/task/sesi_3_hasil.html").then(
      (res) => res.text(),
    ),
    fetch("component/home/detail_pendaftaran/detail_diterima.html").then(
      (res) => res.text(),
    ),
    fetch("component/home/detail_pendaftaran/detail_ditolak.html").then((res) =>
      res.text(),
    ),
    fetch("component/programku/programku.html").then((res) => res.text()),
    fetch("component/sertifikatku/tab_pills.html").then((res) => res.text()),
    fetch("component/sertifikatku/summary_header.html").then((res) =>
      res.text(),
    ),
...
window.renderDetailStatusPendaftaran = function (index, asalHalaman = "Home") {
  ...
  if (data.status === "diterima") {
    ...
    document.getElementById("detail-judul-terima").textContent =
      data.judul + " " + (data.subJudul || "");
    ...
    document.getElementById("materi-injection-point").innerHTML =
      window.AppTemplates.progMateri;
    initProgramMateriLogic();
    if (window.selectProgramById) window.selectProgramById(data.id);
  } else {
    appContent.innerHTML = window.AppTemplates.detailDitolak;

    document.getElementById("detail-judul-tolak").textContent =
      data.judul + " " + (data.subJudul || "");
